  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await updatePost(post._id, {
        ...
        // Send newly selected images or omit field to keep existing logic
        // Depending on backend, passing undefined leaves old images intact
      ...
      if (response.success) {
        ...
        setErrors({ submit: response.message || 'Failed to update post. Please try again.' });
      }
    } catch (error: any) {
          ...
          <div className="flex flex-wrap gap-2">
            {postTypes.map((type) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-lg border transition-colors ${postType === type.value
